...
public class SteamVR_Controller
{
    ...
    public class Device
    {
        ...
        double msecPerFrame = 1000 / 90; // 1000 msec/sec over 90 frames/sec
        public void Update()
        {
            ...
            int frameCount = Convert.ToInt32(frameCounter.ElapsedMilliseconds / msecPerFrame);
            if (frameCount != prevFrameCount)
            {
            	...
            	if (system != null)
            	{
                    uint vrControllerState_t_size = (uint)System.Runtime.InteropServices.Marshal.SizeOf(state);
            		valid = system.GetControllerStateWithPose(ETrackingUniverseOrigin.TrackingUniverseStanding, index, ref state, vrControllerState_t_size, ref pose);
            		UpdateHairTrigger();
            	}
            }
        }
        ...
        //public Matrix Transform { get { Update(); return new Vector3(PositionX, PositionY, PositionZ); } }
    }
		...
		int relativeTo = (int)OpenVR.k_unTrackedDeviceIndex_Hmd) // use -1 for absolute tracking space
	{
		...
		for (int i = 0; i < OpenVR.k_unMaxTrackedDeviceCount; i++)
		{
			...
			{
                Vector3 v3Forward = new Vector3(0, 0, 1);
				var dir = SteamVR_Utils.Normalize(new Vector3(pos.X, 0.0f, pos.Z));
				var dot = Vector3.Dot(dir, v3Forward);
				var cross = Vector3.Cross(dir, v3Forward);
